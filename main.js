...
function installCert(revertOnFail = false) {
  ...
  execFile('powershell.exe', ['-Command', ps], { windowsHide: true }, err => {
    if (err) {
      ...
      safeSend('status-update', 'SlopBlock is running');
...
function toggleVideoDetection() {
  ...
  logger.debugLog(`Video detection ${state.VIDEO_DETECTION_ENABLED ? 'enabled' : 'disabled'}`);
  if (state.VIDEO_DETECTION_ENABLED && !videoClassifier.isVideoClassifierReady()) {
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    safeSend('model-counts', {
      text:  classifier.TEXT_MODEL_TOTAL,
      image: classifier.getImageEnsembleTotal(),
...
app.whenReady().then(async () => {
  ...
  state.textAnalyzed      = savedCounts.textAnalyzed      || 0;
  state.imagesAnalyzed    = savedCounts.imagesAnalyzed    || 0;
  state.youtubeAnalyzed   = savedCounts.youtubeAnalyzed   || 0;
...
ipcMain.on('toggle-video-detection', toggleVideoDetection);
...
ipcMain.on('reset-all', () => {
  ...
  state.textAnalyzed    = 0;
  state.imagesAnalyzed  = 0;
  state.youtubeAnalyzed = 0;
  counts.flush(state);
  safeSend('filter-count',    0);
  safeSend('images-count',    0);
  safeSend('youtube-count',   0);
  safeSend('text-analyzed',   0);
  safeSend('images-analyzed', 0);
  safeSend('youtube-analyzed',0);
