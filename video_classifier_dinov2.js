...
const { AutoProcessor, AutoModel, RawImage, env } = require("@huggingface/transformers");
...
let processor = null;    // DINOv2 image processor
let model = null;        // DINOv2 ONNX model
let probe = null;        // parsed probe JSON
let ready = false;
let _useGPU = false;     // GPU toggle state

// ─── GPU helper ──────────────────────────────────────────────────────────────

function _applyExecutionProvider() {
  if (_useGPU) {
    env.backends.onnx.executionProviders = ["dml"];
    console.log("[VideoClassifier] Execution provider: DirectML (GPU)");
  } else {
    env.backends.onnx.executionProviders = ["cpu"];
    console.log("[VideoClassifier] Execution provider: CPU");
  }
}
...
async function init() {
  ...
  // Ensure EP matches current GPU toggle
  _applyExecutionProvider();
  ...
  processor = await AutoProcessor.from_pretrained(DINOV2_MODEL, {
    revision: "main",
  });
  model = await AutoModel.from_pretrained(DINOV2_MODEL, {
    revision: "main",
    quantized: false,
    dtype: "fp32",
  });
...
async function embedFrames(frames) {
  ...
  for (const frameB64 of frames) {
    ...
    // last_hidden_state.data is Float32Array of shape (1, seq_len, 384)
    const data = last_hidden_state.data;
    const cls = Array.from(data.slice(0, 384));  // CLS token at index 0
...
async function loadVideoModel(modelDir) {
  ...
  try {
    ...
    console.error("[VideoClassifier] loadVideoModel failed:", err.message);
    return false;
  }
}
...
function getVideoThreshold() {
  return probe?.threshold ?? 0.5;
}

function getVideoWarnThreshold() {
  return (probe?.threshold ?? 0.5) - 0.15;
}
...
async function embedVideoBuffers(buffers) {
  // Convert Node.js Buffer objects to base64 data URIs for the pipeline
  const frames = buffers.map((b) => {
    const base64 = b.toString("base64");
    return `data:image/jpeg;base64,${base64}`;
  });
  return await embedFrames(frames);
}

async function classifyVideoFrames(buffers) {
  if (!isVideoClassifierReady()) {
    return { score: 0, confidence: 0, method: "dinov2-restrav", skipped: true };
  }
  const frames5 = buffers.map((b) => {
    const base64 = b.toString("base64");
    return `data:image/jpeg;base64,${base64}`;
  });
  const result = await classifyVideo(frames5, null);
  const isAiVideo = result.label === "AI_GENERATED";
  return {
    score: result.score,
    confidence: Math.round(result.score * 100),
    method: `dinov2-${result.phase.toLowerCase()}`,
    skipped: false,
    phase: result.phase,
...
const DEFAULT_CLIP_MODEL = "dinov2-small";

module.exports = {
  classifyVideo,
  init,
  startServer,
  // Backward compatibility with existing SlopBlock service.js / main.js
  loadVideoModel,
