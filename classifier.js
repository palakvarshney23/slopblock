...
async function isAiImage(imageUrl) {
  ...
  if (urlScore !== null) {
    return { score: urlScore, style: 'unknown' };
  }
...
async function isAiImageFromBuffer(buffer) {
  if (!_isImageModelReady) return { score: 0, style: 'unknown' };
  if (!buffer?.length) return { score: 0, style: 'unknown' };
  return new Promise((resolve, reject) => {
    IMAGE_QUEUE.push({ buffer, resolve, reject });
    _drainImageQueue();
  });
}
...
function _drainImageQueue() {
  ...
  const job = IMAGE_QUEUE.shift();
  activeImageJobs++;
  const run = job.buffer
    ? _runImageClassifyBuffer(job.buffer)
    : _runImageClassify(job.imageUrl);
  run
    .then(job.resolve)
    .catch(job.reject)
...
async function _runImageClassify(imageUrl) {
  ...
  return _classifyImageBuffer(buf, cacheKey, imageUrl, imageUrl);
}

async function _runImageClassifyBuffer(buffer) {
  const cacheKey = 'buf:' + crypto.createHash('sha256').update(buffer).digest('hex').slice(0, 24);
  return _classifyImageBuffer(buffer, cacheKey, 'video frame', null, { videoFrame: true });
}

async function _classifyImageBuffer(buf, cacheKey, logLabel, urlFallback = null, options = {}) {
  if (imageClassificationCache.has(cacheKey)) {
    return imageClassificationCache.get(cacheKey);
  }
  ...
  try {
    ...
    if (buf) {
      ...
      if (c2pa === 'ai_generated') {
        debugLog(`C2PA: AI-generation manifest confirmed — ${String(logLabel).slice(0, 80)}`);
    ...
    if (buf) {
      ...
      if (pngChunk === 'ai_generated') {
        debugLog(`PNG chunks: AI generator parameters found — ${String(logLabel).slice(0, 80)}`);
    ...
    if (_isScreenshotFromShared(shared)) {
      debugLog(`[Router] Screenshot detected — skipping ML — ${String(logLabel).slice(0, 60)}`);
      const result = { score: 0, style: 'screenshot', skipped: true };
      imageClassificationCache.set(cacheKey, result);
      return result;
    }
    ...
    // Video frames: blank/flat intros produce unreliable ML scores — skip early.
    if (options.videoFrame && lapScore < 0.12) {
      debugLog(`[VideoFrame] Flat frame (Lap=${Math.round(lapScore * 100)}%) — skipping ML`);
      const result = { score: 0, style: 'flat', skipped: true };
      imageClassificationCache.set(cacheKey, result);
      return result;
    }
    ...
    if (hasCamera && lapScore > 0.60) {
      debugLog(`[Veto] Camera EXIF + high Lap → definitely real — ${String(logLabel).slice(0, 60)}`);
    ...
    debugLog(`[Style] ${style} — ${String(logLabel).slice(0, 60)}`);

    // ── Stage 4: ML detectors ────────────────────────────────────
    let modelInput = urlFallback;
    if (shared) {
      try {
        const { RawImage } = await import('@huggingface/transformers');
        modelInput = new RawImage(new Uint8ClampedArray(shared.px), shared.W, shared.H, 3);
      } catch (_) { /* keep urlFallback when set */ }
    }
    if (!modelInput) {
      const result = { score: 0, style: 'unknown' };
      imageClassificationCache.set(cacheKey, result);
    ...
    // Video frames: require a known AI model label or very strong binary AI score.
    if (options.videoFrame) {
      const knownAiHit = mlResult?.some(r => KNOWN_AI_LABELS.has(r.label) && r.score >= 0.40);
      const strongBinary = bScore >= 0.85 || cScore >= 0.85;
      if (!knownAiHit && !strongBinary) {
        ensembleScore = Math.min(ensembleScore, 0.68);
        debugLog('[VideoFrame] No known AI label — capping ensemble for frame');
      }
    }
    ...
    debugLog(`[Ensemble] Final: ${Math.round(ensembleScore*100)}% style=${style} — ${String(logLabel).slice(0, 60)}`);

    const result = { score: ensembleScore, style, skipped: false };
    imageClassificationCache.set(cacheKey, result);
    return result;
  } catch (err) {
    logError(err);
...
const TEXT_MODEL_TOTAL = 2;

function getImageEnsembleTotal() {
  return 2 + (MODEL_D_ID ? 1 : 0) + 1; // A + B + C [+ D]
}
