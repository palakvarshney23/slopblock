...
const DEFAULTS = {
  ...
  // ── Video detection ────────────────────────────────────────────
  // videoWarnThreshold: minimum confidence (0–1) to flag a video as AI-generated.
  // User-adjustable in Settings. Lower = more sensitive.
  videoWarnThreshold:   0.55,
...
module.exports = {
  ...
  set(key, value) {
    ...
    if (key === 'videoWarnThreshold' || key === 'videoBlockThreshold') {
      const num = Number(value);
      if (Number.isNaN(num)) return false;
      value = Math.min(0.95, Math.max(0.10, num));
    }
    _current[key] = value;
    _write();
    return true;
